...
class DoubleNdTreeTest {
  ...
  private static void assertComputeKnn(double[][] data) {
    ...
    // Create the KDtree
    final DoubleKdTree tree =
        KdTrees.builder(2).setSplitStratgey(SplitStrategy.MEDIAN).buildDouble();
    Assertions.assertEquals(2, tree.dimensions());
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (final int k : KdTreeTestUtils.ks) {
        ...
        final double[] observed = distances.toDoubleArray();
      ...
      for (final double range : KdTreeTestUtils.ranges) {
        ...
        for (int j = 0; j < distances.size(); j++) {
          Assertions.assertTrue(distances.getDouble(j) <= range);
        }
      }
    }
  }
  ...
  void testSingularityWithDistanceAboveRange() {
    ...
    // Create the KDtree
    final DoubleKdTree tree = KdTrees.builder(2).setSplitStratgey(SplitStrategy.MEAN).buildDouble();
    Assertions.assertEquals(2, tree.dimensions());
    for (final double[] location : data) {
      tree.add(location);
    }
  ...
  private static void assertAddIfAbsent(int dim) {
    final DoubleKdTree tree = KdTrees.newDoubleKdTree(dim);
    final int n = dim - 1;
    for (int i = 0; i < 30; i++) {
