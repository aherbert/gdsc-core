...
class ObjDoubleNdTreeTest {
  ...
  private static void assertComputeKnn(double[][] data) {
    ...
    // Create the KDtree
    final ObjDoubleKdTree<Integer> tree =
        KdTrees.builder(2).setSplitStratgey(SplitStrategy.MEDIAN).buildObjDouble();
    Assertions.assertEquals(2, tree.dimensions());
    int item = 0;
  ...
  void testSingularityWithDistanceAboveRange() {
    ...
    // Create the KDtree
    final ObjDoubleKdTree<Integer> tree =
        KdTrees.builder(2).setSplitStratgey(SplitStrategy.MEAN).buildObjDouble();
    Assertions.assertEquals(2, tree.dimensions());
    int item = 0;
    for (final double[] location : data) {
      tree.add(location, Integer.valueOf(item++));
  ...
  private static void assertAddIfAbsent(int dim) {
    final ObjDoubleKdTree<Integer> tree = KdTrees.newObjDoubleKdTree(dim);
    final int n = dim - 1;
    for (int i = 0; i < 30; i++) {
      final double[] point = new double[dim];
      point[n] = i;
      Assertions.assertTrue(tree.addIfAbsent(point, null));
      Assertions.assertEquals(i + 1, tree.size(), () -> "Incorrect size. dim=" + dim);
      Assertions.assertFalse(tree.addIfAbsent(point.clone(), null),
          () -> "Point added. dim=" + dim);
      Assertions.assertEquals(i + 1, tree.size(), () -> "Incorrect size. dim=" + dim);
    }
    ...
    final double[] point = new double[dim];
    point[n] = -0f;
    Assertions.assertFalse(tree.addIfAbsent(point, null));
  }
