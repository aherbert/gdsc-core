...
class IntFloatNdTree implements IntFloatKdTree {
   ...
   */
  private IntFloatNdTree(IntFloatNdTree parent, float[][] locations, int[] data,
      int locationCount) {
  ...
  public boolean addIfAbsent(float[] location, int value) {
    final BiPredicate<float[], float[]> equality = FloatArrayPredicates.equalsForLength(dimensions);
   ...
   */
  private boolean addPoint(float[] location, int value,
      BiPredicate<IntFloatNdTree, float[]> filter) {
    ...
    if (cursor.locationCount == cursor.locations.length) {
      ...
      if (cursor.minLimit[cursor.splitDimension] == cursor.maxLimit[cursor.splitDimension]) {
        ...
        if (left == size) {
          ...
          cursor.left = new IntFloatNdTree(cursor, leftLocations, leftData, left);
          cursor.right = new IntFloatNdTree(cursor, rightLocations, rightData, right);
          ...
          int depth = 1;
          for (IntFloatNdTree parent = cursor.parent; parent != null; parent = parent.parent) {
            depth++;
          }
   ...
   * @return the child
   */
  private static IntFloatNdTree updateAndDescend(IntFloatNdTree current, float[] location) {
   ...
   */
  private static void ascendAndDecrement(IntFloatNdTree current) {
