...
class OffsetPolygonRoiTest {
  ...
  void testConstructor() {
    ...
    // A better test would be to compare to the standard PolygonRoi when on an image canvas
    // that the screenXD/offScreenXD/etc methods do not add a 0.5 offset.
    Assertions
        .assertFalse(new OffsetPolygonRoi(new float[] {0.5f}, new float[] {1.5f}, Roi.POLYLINE)
            .useLineSubpixelConvention());
    Assertions
        .assertFalse(new OffsetPolygonRoi(new float[] {0.5f}, new float[] {1.5f}, 1, Roi.POLYLINE)
            .useLineSubpixelConvention());
  }
}
