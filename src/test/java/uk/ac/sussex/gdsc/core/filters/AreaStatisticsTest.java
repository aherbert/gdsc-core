...
public class AreaStatisticsTest {
  ...
  public void canComputeGlobalStatistics(RandomSeed seed) {
    ...
    for (final boolean r : rolling) {
      ...
      TestAssertions.assertTest(s.getStandardDeviation(), o[AreaStatistics.SD], TestHelper.almostEqualDoubles(1e-6, 0));
    ...
    for (final int x : Random.sample(5, maxx, r)) {
      for (final int y : Random.sample(5, maxy, r)) {
        for (final int n : boxSizes) {
          ...
          TestAssertions.assertTest(s.stdDev, o[AreaStatistics.SD], TestHelper.almostEqualDoubles(1e-6, 0));
        }
      }
    }
  }
  ...
  public void canComputeStatisticsWithinClippedBounds() {
    ...
    for (final boolean r : rolling) {
      ...
      for (final int n : boxSizes) {
        ...
        TestAssertions.assertTest(s, o[AreaStatistics.SD], TestHelper.almostEqualDoubles(1e-6, 0));
