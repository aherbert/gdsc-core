...
public final class CircularKernelOffset {
   ...
   * @param startInternal the start internal
  ...
  /**
   * Create the offsets for a circular kernel. This represents a circular kernel around a central
   * pixel with size 2 * resolution + 1. The centre pixel is (0,0), returned as row index
   * [resolution] in the final array. The offsets are returned so that the following is valid to
   * iterate over all the pixels in a row of the kernel:
   *
   * <pre>
   * int x,y = ...;
  ...
  public static CircularKernelOffset[] create(int resolution) {
    ...
    // The outer pixel outer corner (1,1) must be compared to the farthest point in the origin
    // pixel:
    ...
    for (int y = 0; y <= resolution; y++) {
      ...
      // Move the inner pixel if it is above the limit and the maximum distance is not currently
      // inside.
