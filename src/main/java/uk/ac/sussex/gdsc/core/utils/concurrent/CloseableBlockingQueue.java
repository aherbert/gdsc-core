...
public class CloseableBlockingQueue<E> extends AbstractQueue<E>
  ...
  /**
   * Creates an {@code CloseableBlockingQueue} with the given (fixed) capacity, the specified access
   ...
   * @throws IllegalStateException if this queue is full or closed
  ...
  class Itrs {
     ...
     *
     * @param initial the initial iterator
     */
    Itrs(Itr initial) {
      register(initial);
    }
    ...
    /**
     * Sweeps itrs, looking for and expunging stale iterators. If at least one was found, tries
     * harder to find more. Called only from iterating thread.
     *
  ...
  private class Itr implements Iterator<E> {
    ...
    /**
     * Special index value indicating "removed elsewhere", that is, removed by some operation other
     * than a call to this.remove().
     */
    ...
    /**
     * Returns true if index is invalidated by the given number of dequeues, starting from
     * prevTakeIndex.
     */
    ...
    /**
     * Adjusts indices to incorporate all dequeues since the last operation on this iterator. Call
     * only from iterating thread.
     */
    ...
    /**
     * Called when itrs should stop tracking this iterator, either because there are no more indices
     * to update (cursor < 0 && nextIndex < 0 && lastRet < 0) or as a special exception, when
     * lastRet >= 0, because hasNext() is about to return false for the first time. Call only from
     * iterating thread.
     */
    private void detach() {
     ...
     * To allow for this, we only access fields (i.e. nextItem) that are not modified by update
     * operations triggered by queue modifications.
     */
    @Override
    ...
    /**
     * Called to notify the iterator that the queue is empty, or that it has fallen hopelessly
     * behind, so that it should abandon any further iteration, except possibly to return one more
     * element from next(), as promised by returning true from hasNext().
     */
    void shutdown() {
      // assert lock.getHoldCount() == 1;
      cursor = NONE;
