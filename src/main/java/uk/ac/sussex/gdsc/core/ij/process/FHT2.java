...
public class FHT2 extends FloatProcessor {
  ...
  /**
   * FFT real & imaginary value of one row from 2D Hartley Transform. Author: Joachim Wesner Adapted
   * by Alex Herbert to compute both together
   *
   * @param row the row
   * @param maxN the max N
   * @param fht the fht
  ...
  /**
   * Initialise fast operations for {@link #multiply(FHT2)} and {@link #conjugateMultiply(FHT2)}.
   * This pre-computes the values needed for the operations. <p> Note: This initialises the FHT
   * object for use as the argument to the operation, for example if a convolution kernel is to be
   * applied to many FHT objects.
   */
   ...
   * {@link #divide(FHT2)}. This pre-computes the values needed for the operations. <p> Note: This
   * initialises the FHT object for use as the argument to the operation, for example if a
   * deconvolution kernel is to be applied to many FHT objects.
   */
