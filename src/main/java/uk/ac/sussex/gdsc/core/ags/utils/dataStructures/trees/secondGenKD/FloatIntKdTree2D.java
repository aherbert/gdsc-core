 ...
 * dimensions. Functionality to limit the tree size has been removed. The status property has been
 * removed to allow multi-threaded search.
 *
 * @author Alex Herbert
 */
public abstract class FloatIntKdTree2D extends FloatIntKdTreeNode2D {
    ...
    while (cursor.locations == null || cursor.locationCount >= cursor.locations.length) {
      if (cursor.locations != null) {
        ...
        if (cursor.minLimit[cursor.splitDimension] == cursor.maxLimit[cursor.splitDimension]) {
          ...
          final int[] newData = new int[newLocations.length];
        ...
        // Create child leaves
        final FloatIntKdTreeNode2D left = new ChildNode(cursor, nextId++);
        final FloatIntKdTreeNode2D right = new ChildNode(cursor, nextId++);

        // Move locations into children
        for (int i = 0; i < cursor.locationCount; i++) {
          final float[] oldLocation = cursor.locations[i];
          final int oldData = cursor.data[i];
   ...
   * @param sequentialSorting Set to true to return the points in order, largest distance first
   * @return the list
   */
   ...
   * @param count the count
   * @param sequentialSorting Set to true to return the points in order, largest distance first
   * @param status the status array. Must equal the number of nodes in the tree
   * @return the list
   */
   ...
   * distance checking <p> This is an optimised version for use in the GDSC Core project.
   *
   * @author Alex Herbert
   */
