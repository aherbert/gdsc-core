...
public class Random extends AbstractRandomGenerator {
   ...
   * @param r the random generator
   */
  public static void shuffle(double[] data, UniformRandomProvider r) {
    for (int i = data.length; i-- > 1;) {
      final int j = r.nextInt(i + 1);
      final double tmp = data[i];
      data[i] = data[j];
   ...
   * @param r the random generator
   */
  public static void shuffle(float[] data, UniformRandomProvider r) {
    for (int i = data.length; i-- > 1;) {
      final int j = r.nextInt(i + 1);
      final float tmp = data[i];
      data[i] = data[j];
   ...
   * @param r the random generator
   */
  public static void shuffle(int[] data, UniformRandomProvider r) {
    for (int i = data.length; i-- > 1;) {
      final int j = r.nextInt(i + 1);
      final int tmp = data[i];
      data[i] = data[j];
   ...
   * @param r the random generator
   */
  public static void shuffle(Object[] data, UniformRandomProvider r) {
    for (int i = data.length; i-- > 1;) {
      final int j = r.nextInt(i + 1);
      final Object tmp = data[i];
      data[i] = data[j];
   ...
   * shuffle on an array of length n for the first k target indices. <p> Note: Returns an empty
   * array if n or k are less than 1. Returns an ascending array of indices if k is equal or bigger
   * than n.
   *
   * @param k the k
   * @param n the n
  ...
  /**
   * Sample k values without replacement from the data. <p> Note: Returns an empty array if k is
   * less than 1. Returns a copy of the data if k is greater than data.length.
   *
   * @param k the k
   * @param data the data
   * @return the sample
   ...
   * @return the sample
   */
  public static int[] sample(final int k, final int n, UniformRandomProvider r) {
    // Avoid stupidity
    if (n < 1 || k < 1) {
      return new int[0];
    }
  ...
  private static int[] inlineSelection(final int[] data, int k, UniformRandomProvider r) {
    // Do an in-line Fisher-Yates shuffle into a result array
    final int[] result = new int[k];
    for (int i = data.length - 1; k-- > 0; i--) {
      final int j = r.nextInt(i + 1);
   ...
   * @return the sample
   */
  public static int[] sample(final int k, final int[] data, UniformRandomProvider r) {
    final int[] sample = sample(k, data.length, r);
    // Convert indices to values
    for (int i = sample.length; i-- > 0;) {
      sample[i] = data[sample[i]];
