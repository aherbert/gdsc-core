...
public class RollingArrayMoment implements ArrayMoment {
  ...
  /**
   * {@inheritDoc} <p> Note: If the user desires to maintain just a single moment then it advised to
   * use the Apache class org.apache.commons.math3.stat.descriptive.moment.SecondMoment. </p> <p>
   * For each additional value, update the first moment using:<br> <code>m = m + (new value - m) /
   * (number of observations)</code> </p> <p> The following recursive updating formula is used for
   * the second moment: </p> <p> Let <ul> <li>dev = (current obs - previous mean)</li> <li>n =
   * number of observations (including current obs)</li> </ul> <p> Then </p> <p> new value = old
   * value + dev^2 * (n -1) / n. </p>
   *
   * @param data the data
   */
