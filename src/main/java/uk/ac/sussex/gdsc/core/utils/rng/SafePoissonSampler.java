...
/**
 * Class for computing Poisson samples handling the edge case where the mean is
 * zero. In that case the sample return is zero.
 */
public class SafePoissonSampler {

    /**
     * Value for switching sampling algorithm. This is the same pivot point as the
     * PoissonSampler
     */
     ...
     * <p>
     * If the mean is zero this will return a sampler that always returns 0.
     *
    ...
    public static DiscreteSampler createPoissonSampler(UniformRandomProvider rng, double mean)
        ...
        if (mean == 0)
            return ZERO;
        return mean < PIVOT ? new SmallMeanPoissonSampler(rng, mean) : new LargeMeanPoissonSampler(rng, mean);
    }

    /**
     * Creates a new Poisson sample.
     * <p>
     * If the mean is zero this will return 0.
     *
     * @param rng  Generator of uniformly distributed random numbers.
     * @param mean Mean of the Poisson distribution.
     ...
     * @throws IllegalArgumentException if {@code mean < 0}.
     */
    public static int nextPoissonSample(UniformRandomProvider rng, double mean) throws IllegalArgumentException {
        if (mean == 0)
            return 0;
