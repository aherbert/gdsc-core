...
/**
 * Compute clustering using OPTICS. <p> This is an implementation of the OPTICS method. Mihael
 * Ankerst, Markus M Breunig, Hans-Peter Kriegel, and Jorg Sander. Optics: ordering points to
...
public class OPTICSManager extends CoordinateStore {
  ...
  public enum Option {
     ...
     * benefits since some distance computations can be avoided. <p> Note: This option is
     * experimental and can cause slow-down
     */
    CIRCULAR_PROCESSING,
    /**
     * Flag to indicate that inner-circle processing should be used on the 2D grid. This has
     * performance benefits for DBSCAN since some distance computations can be assumed. <p> Note:
     * This option is experimental and can cause slow-down
     */
    INNER_PROCESSING,
    /**
     * Flag to indicate that processing should use a 2D grid. <p> Note: If no options are provided
     * then the memory structure will be chosen automatically.
     */
    GRID_PROCESSING,
    /**
     * Flag to indicate that OPTICS should sort objects using their input order (id) if the
     * reachability distance is equal. <p> Omitting this flag defaults to sorting objects using
     * their reachability distance. If the reachability distance is the same then the order will be
     * dependent on the type of priority queue, i.e. is non-deterministic.
     */
     ...
     * reachability distance is equal. <p> Omitting this flag defaults to sorting objects using
     * their reachability distance. If the reachability distance is the same then the order will be
     * dependent on the type of priority queue, i.e. is non-deterministic. <p> Note: This option
     * matches the implementation in the ELKI framework.
     */
  ...
  /**
   * Used in the OPTICS algorithm to store the next seed is a priority queue <p> If distances are
   * equal then IDs are used to sort the objects in order.
   */
  ...
  /**
   * Used in the OPTICS algorithm to store the next seed is a priority queue <p> If distances are
   * equal then IDs are used to sort the objects in reverse order.
   */
  ...
  /**
   * An implementation of a binary heap respecting minimum order. <p> This class is based on
   * ags.utils.dataStructures.BinaryHeap
   */
  ...
  /**
   * An implementation of a binary heap respecting minimum order. <p> If distances are equal then
   * IDs are used to sort the objects in order.
   */
  ...
  /**
   * An implementation of a binary heap respecting minimum order. <p> If distances are equal then
   * IDs are used to sort the objects in reverse order.
   */
   ...
   * reachability distance of a point from another core point. <p> This is an implementation of the
   * OPTICS method. Mihael Ankerst, Markus M Breunig, Hans-Peter Kriegel, and Jorg Sander. Optics:
   * ordering points to identify the clustering structure. In ACM Sigmod Record, volume 28, pages
   * 49–60. ACM, 1999. <p> The returned results are the output of
   * {@link OPTICSResult#extractDBSCANClustering(float)} with the configured generating distance.
   * Note that the generating distance may have been modified if invalid. If it is not strictly
   ...
   * the results. <p> This creates a large memory structure. It can be held in memory for re-use
   * when using a different number of min points. The tracker can be used to follow progress (see
  ...
  public OPTICSResult optics(float generatingDistanceE, int minPts) {
    ...
    // Perhaps the reachability distance profile is more consistent when changing the generating
    // distance?
   ...
   */
  private boolean expandClusterOrder(Molecule object, float e, int minPts,
   ...
   * @param centreObject the object
   */
  private static void update(OPTICSPriorityQueue orderSeeds, MoleculeList neighbours,
      Molecule centreObject) {
    final float c_dist = centreObject.coreDistance;
    for (int i = neighbours.size; i-- > 0;) {
      final Molecule object = neighbours.get(i);
   ...
   * @param N the number of points in the data space
   * @return the generating distance
   */
  public static float computeGeneratingDistance(int minPts, double area, int N) {
    ...
    final double volumeS = (volumeDS / N) * k;
   ...
   * the search space. <p> This is an implementation of the DBSCAN method. Ester, Martin; Kriegel,
   * Hans-Peter; Sander, Jörg; Xu, Xiaowei (1996). Simoudis, Evangelos; Han, Jiawei; Fayyad, Usama
   * M., eds. A density-based algorithm for discovering clusters in large spatial databases with
   * noise. Proceedings of the Second International Conference on Knowledge Discovery and Data
   * Mining (KDD-96). AAAI Press. pp. 226–231. <p> Note that the generating distance may have been
   ...
   * distance is set to 1. The distance is stored in the results. <p> This creates a large memory
   * structure. It can be held in memory for re-use when using a different number of min points. The
   * tracker can be used to follow progress (see {@link #setTracker(TrackProgress)}).
   *
   * @param generatingDistanceE the generating distance (E) (set to zero to auto calibrate)
   * @param minPts the min points for a core object
   * @return the results (or null if the algorithm was stopped using the tracker)
    ...
    if (!stopped) {
      ...
      if (tracker != null) {
        ...
        tracker.log("Finished DBSCAN: %s (Time = %s)",
            TextUtils.pleural(counter.getTotalClusters(), "Cluster"), Utils.timeToString(time));
      }
    }
  ...
  private boolean expandCluster(Molecule object, float e, int minPts, Counter counter,
      MoleculeQueue seeds) {
   ...
   */
  private static void update(MoleculeQueue pointsToSearch, MoleculeList neighbours, int clusterId) {
    for (int i = neighbours.size; i-- > 0;) {
      final Molecule object = neighbours.get(i);
   ...
   * automatically using a % noise threshold. <p> See: Jörg Sander, Martin Ester, Hans-Peter
   * Kriegel, Xiaowei Xu Density-Based Clustering in Spatial Databases: The Algorithm GDBSCAN and
   * Its Applications Data Mining and Knowledge Discovery, 1998.
   *
   * @param k the k (automatically bounded between 1 and size-1)
   ...
   * Compute the core radius for each point to have n closest neighbours and the minimum
   * reachability distance of a point from another core point. <p> This is an implementation of the
   * Fast OPTICS method. J. Schneider and M. Vlachos. Fast parameterless density-based clustering
   * via random projections. Proc. 22nd ACM international conference on Conference on Information
   * &amp; Knowledge Management (CIKM). <p> Fast OPTICS uses random projections of the data into a
   * linear space. The linear space is then divided into sets until each set is below a minimum
   * size. The process is repeated multiple times to generate many sets of neighbours. Analysis for
   * each object is then performed using only those other objects that are found in the same sets as
   * it (the union of the neighbours in all sets containing an object defines the object
   * neighbourhood). This method therefore does not require a distance threshold as each object will
   * always have a neighbourhood and a core distance which is the average distance to all the other
   * objects in its neighbourhood. The method allows clustering of data with widely varying
   * densities across the set. <p> Note that once the neighbourhood and core distance are computed
   * for each object the remaining algorithm follows that of OPTICS. Note that despite the name this
   * is not faster than the OPTICS method implemented here due to the low number of dimensions.
   * Speed is significantly faster for high dimensional data. <p> The returned results are the
   * output of {@link OPTICSResult#extractDBSCANClustering(float)} with the auto-configured
   * generating distance set using {@link #computeGeneratingDistance(int)}. If it is larger than the
   * data range allows it is set to the maximum distance that can be computed for the data range. If
   * the data are colocated the distance is set to 1. The distance is stored in the results. <p>
   * This implementation is a port of the version in the ELKI framework:
   ...
   * @return the results (or null if the algorithm was stopped using the tracker)
   */
  public OPTICSResult fastOptics(int minPts, int nSplits, int nProjections,
   ...
   * expected value of 0 (not an outlier). <p> See: Hans-Peter Kriegel, Peer Kröger, Erich Schubert,
   * Arthur Zimek:<br> LoOP: Local Outlier Probabilities<br> In Proceedings of the 18th
   * International Conference on Information and Knowledge Management (CIKM), Hong Kong, China, 2009
   *
   * @param k the number of neighbours (automatically bounded between 1 and size-1)
  ...
  public float[] loop(int k, double lambda, boolean cache) {
    ...
    if (tracker != null) {
      tracker.log("Computing Local Outlier Probability scores, k=%d, Lambda=%s", k,
          Utils.rounded(lambda));
    }
