...
public class IntMedianWindow {
   ...
   */
  IntMedianWindow(int[] data, int radius) {
    if (radius < 0) {
   ...
   * @throws IllegalArgumentException if the radius is negative
   */
  public static IntMedianWindow wrap(int[] data, int radius) {
    return new IntMedianWindow(data, radius);
  }
  ...
  private float updateMedian() {
    ...
    if (cache == null || position - cachePosition > radius / 2 || cache.length != newLength) {
      ...
      if (sortedScan) {
        ...
        final int[] dataToRemove = new int[position - cachePosition];
