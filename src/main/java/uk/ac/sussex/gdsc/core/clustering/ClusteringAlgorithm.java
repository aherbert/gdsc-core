...
public enum ClusteringAlgorithm {
  /**
   * Joins the closest pair of particles, one of which must not be in a cluster. Clusters are not
   * joined and can only grow when particles are added.
   */
  PARTICLE_SINGLE_LINKAGE {
    @Override
  ...
  /**
   * Join the current set of closest pairs in a greedy algorithm. This method computes the pairwise
   * distances and joins the closest pairs without updating the centroid of each cluster, and the
   * distances, after every join (centroids and distances are updated after each pass over the
   * data). This can lead to errors over true hierarchical centroid-linkage clustering where
   * centroid are computed after each link step. For example if A joins B and C joins D in a single
   * step but the new centroid of AB is closer to C than D.
   */
  PAIRWISE {
    @Override
    public String getName() {
   ...
   *
   * <p>In dense images this will return the same results as the Closest algorithm but will be much
   * slower. It may be faster for sparse density due to the greedy nature of the algorithm.
   */
  PAIRWISE_WITHOUT_NEIGHBOURS {
    @Override
    public String getName() {
  ...
  /**
   * Hierarchical centroid-linkage clustering by joining the closest pair of clusters iteratively.
   * Clusters are compared using time and distance thresholds with priority on the closest time gap
   * (within the distance threshold).
   */
  CENTROID_LINKAGE_DISTANCE_PRIORITY {
    @Override
    public String getName() {
  ...
  /**
   * Hierarchical centroid-linkage clustering by joining the closest pair of clusters iteratively.
   * Clusters are compared using time and distance thresholds with priority on the closest distance
   * gap (within the time threshold).
   */
  CENTROID_LINKAGE_TIME_PRIORITY {
    @Override
    public String getName() {
