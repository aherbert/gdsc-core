...
public class Maths {
   ...
   * @param p The number of fitted parameters
  ...
  public static double getLogLikelihood(double sumOfSquaredResiduals, int n) {
    // final double logLikelihood = 0.5 * (-n * Math.log(2 * Math.PI) - n *
    // Math.log(sumOfSquaredResiduals/n) - n);
    // final double logLikelihood = 0.5 * (-n * (Math.log(2 * Math.PI) +
    // Math.log(sumOfSquaredResiduals/n) + 1));
    // Math.log(2 * Math.PI) = 1.837877066
    final double logLikelihood =
        0.5 * (-n * (1.837877066 + Math.log(sumOfSquaredResiduals / n) + 1.0));
   ...
   *
   * @param residualSS The sum of squared residuals from the model
   * @param totalSS the sum of the squared differences from the mean of the dependent variable
   *        (total sum of squares)
   * @param n The number of data points
   * @param p The number of fitted parameters
   * @return The adjusted coefficient of determination
   */
  public static double getAdjustedCoefficientOfDetermination(double residualSS, double totalSS,
      int n, int p) {
   ...
   * otherwise (for NaN and infinity arguments). <p> Taken from Double in the standard java
   * implementation. This method was added in 1.8.
   *
  ...
  /**
   * Checks if is a power of 2. Note a value of 1 will return true (as this is 2^0). <p> Adapted
   * from the JTransforms library class org.jtransforms.utils.CommonUtils.
   *
   * @param x the x
   ...
   * JTransforms library class org.jtransforms.utils.CommonUtils.
   *
   * @param x the x
  ...
  /**
   * Return the log2 of x rounded down to a power of 2. This is done by scanning for the most
   * significant bit of the value. <p> If x is negative or zero this will return Integer.MIN_VALUE
