 ...
 *
 * <p>This is the same algorithm as the {@link KuhnMunkresAssignment} adapted for floating-point
 ...
 * the input cost matrix to integer costs and using the {@link KuhnMunkresAssignment} class.
 *
...
public class DoubleKuhnMunkresAssignment {
   ...
   */
  private DoubleKuhnMunkresAssignment(double[] cost, int rows, int cols) {
    this.cost = cost;
    this.rows = rows;
    this.cols = cols;
  }
  ...
  static DoubleKuhnMunkresAssignment create(double[][] cost) {
    ...
    return new DoubleKuhnMunkresAssignment(data, rows, cols);
  }
  ...
  public static int[] compute(double[] cost, int rows, int cols) {
    ...
    return new DoubleKuhnMunkresAssignment(cost, rows, cols).compute();
  }
  ...
  private int step5(boolean[] rowMask, boolean[] colMask) {
    final double h = findSmallestNonCovered(rowMask, colMask);
   ...
   */
  private double findSmallestNonCovered(boolean[] rowMask, boolean[] colMask) {
    double min = Double.POSITIVE_INFINITY;
    for (int row = 0; row < rows; row++) {
      if (!rowMask[row]) {
        for (int col = 0; col < cols; col++) {
          if (!colMask[col]) {
            final double value = cost[row * cols + col];
  ...
  /**
   * Adds the values without overflow to infinity.
   *
   * <p>Note this may not ensure the correct functioning of the algorithm is all cases since it is
   * possible to add two double values and get the same value, e.g. {@link Double#MAX_VALUE} and 1.
   * This case may be better served by mapping floating-point costs to integers and performing the
  ...
  /**
   * Subtracts the values without underflow past zero.
   *
   * <p>Since the algorithm detects zeros in the matrix it is important not to move past 0 to
   * negative.
   *
   * @param value1 the first value
   * @param value2 the second value
