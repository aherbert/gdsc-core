...
public class MedianWindow {
  ...
  /**
   * Instantiates a new median window. <p> Note that the median must be written back to a different
   * array, i.e. the input data should be cloned if the array is to be reused.
   *
   * @param data the data
  ...
  private double updateMedian() {
    ...
    if (cache == null || position - cachePosition > radius / 2 || cache.length != newLength) {
      ...
      if (sortedScan) {
        ...
        final double[] dataToRemove = new double[position - cachePosition];
