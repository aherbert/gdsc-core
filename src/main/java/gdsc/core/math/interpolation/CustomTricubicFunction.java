...
public class CustomTricubicFunction implements TrivariateFunction
{
			...
			double[] d2f_da2)
	{
		...
		result += a[4] * pY[1];
		df_da[0] += pY[1] * a[5];
		d2f_da2[0] += 2 * pY[1] * a[6];
		df_da[1] += 2 * pY[1] * a[8];
		d2f_da2[1] += 6 * pY[1] * a[12];
		df_da[2] += pY[1] * a[20];
		d2f_da2[2] += 2 * pY[1] * a[36];
		pXpYpZ = pX[1] * pY[1];
		result += a[5] * pXpYpZ;
		df_da[0] += 2 * pXpYpZ * a[6];
		...
		result += a[8] * pY[2];
		df_da[0] += pY[2] * a[9];
		d2f_da2[0] += 2 * pY[2] * a[10];
		df_da[1] += 3 * pY[2] * a[12];
		df_da[2] += pY[2] * a[24];
		d2f_da2[2] += 2 * pY[2] * a[40];
		pXpYpZ = pX[1] * pY[2];
		result += a[9] * pXpYpZ;
		df_da[0] += 2 * pXpYpZ * a[10];
		...
		result += a[12] * pY[3];
		df_da[0] += pY[3] * a[13];
		d2f_da2[0] += 2 * pY[3] * a[14];
		df_da[2] += pY[3] * a[28];
		d2f_da2[2] += 2 * pY[3] * a[44];
		pXpYpZ = pX[1] * pY[3];
		result += a[13] * pXpYpZ;
		df_da[0] += 2 * pXpYpZ * a[14];
		...
		result += a[16] * pZ[1];
		df_da[0] += pZ[1] * a[17];
		d2f_da2[0] += 2 * pZ[1] * a[18];
		df_da[1] += pZ[1] * a[20];
		d2f_da2[1] += 2 * pZ[1] * a[24];
		df_da[2] += 2 * pZ[1] * a[32];
		d2f_da2[2] += 6 * pZ[1] * a[48];
		pXpYpZ = pX[1] * pZ[1];
		result += a[17] * pXpYpZ;
		df_da[0] += 2 * pXpYpZ * a[18];
		...
		result += a[32] * pZ[2];
		df_da[0] += pZ[2] * a[33];
		d2f_da2[0] += 2 * pZ[2] * a[34];
		df_da[1] += pZ[2] * a[36];
		d2f_da2[1] += 2 * pZ[2] * a[40];
		df_da[2] += 3 * pZ[2] * a[48];
		pXpYpZ = pX[1] * pZ[2];
		result += a[33] * pXpYpZ;
		df_da[0] += 2 * pXpYpZ * a[34];
		...
		result += a[48] * pZ[3];
		df_da[0] += pZ[3] * a[49];
		d2f_da2[0] += 2 * pZ[3] * a[50];
		df_da[1] += pZ[3] * a[52];
		d2f_da2[1] += 2 * pZ[3] * a[56];
		pXpYpZ = pX[1] * pZ[3];
		result += a[49] * pXpYpZ;
		df_da[0] += 2 * pXpYpZ * a[50];
	...
	private static double[][] computeSecondOrderPowerTables(final double[] pX, final double[] pY, final double[] pZ)
	{
		...
		table[4] = pY[1];
		table_df_dx[3] = pY[1];
		table_d2f_dx2[2] = 2 * pY[1];
		table_df_dy[4] = 2 * pY[1];
		table_d2f_dy2[4] = 6 * pY[1];
		table_df_dz[4] = pY[1];
		table_d2f_dz2[4] = 2 * pY[1];
		pXpYpZ = pX[1] * pY[1];
		table[5] = pXpYpZ;
		table_df_dx[4] = 2 * pXpYpZ;
		...
		table[8] = pY[2];
		table_df_dx[6] = pY[2];
		table_d2f_dx2[4] = 2 * pY[2];
		table_df_dy[8] = 3 * pY[2];
		table_df_dz[8] = pY[2];
		table_d2f_dz2[8] = 2 * pY[2];
		pXpYpZ = pX[1] * pY[2];
		table[9] = pXpYpZ;
		table_df_dx[7] = 2 * pXpYpZ;
		...
		table[12] = pY[3];
		table_df_dx[9] = pY[3];
		table_d2f_dx2[6] = 2 * pY[3];
		table_df_dz[12] = pY[3];
		table_d2f_dz2[12] = 2 * pY[3];
		pXpYpZ = pX[1] * pY[3];
		table[13] = pXpYpZ;
		table_df_dx[10] = 2 * pXpYpZ;
		...
		table[16] = pZ[1];
		table_df_dx[12] = pZ[1];
		table_d2f_dx2[8] = 2 * pZ[1];
		table_df_dy[12] = pZ[1];
		table_d2f_dy2[8] = 2 * pZ[1];
		table_df_dz[16] = 2 * pZ[1];
		table_d2f_dz2[16] = 6 * pZ[1];
		pXpYpZ = pX[1] * pZ[1];
		table[17] = pXpYpZ;
		table_df_dx[13] = 2 * pXpYpZ;
		...
		table[32] = pZ[2];
		table_df_dx[24] = pZ[2];
		table_d2f_dx2[16] = 2 * pZ[2];
		table_df_dy[24] = pZ[2];
		table_d2f_dy2[16] = 2 * pZ[2];
		table_df_dz[32] = 3 * pZ[2];
		pXpYpZ = pX[1] * pZ[2];
		table[33] = pXpYpZ;
		table_df_dx[25] = 2 * pXpYpZ;
		...
		table[48] = pZ[3];
		table_df_dx[36] = pZ[3];
		table_d2f_dx2[24] = 2 * pZ[3];
		table_df_dy[36] = pZ[3];
		table_d2f_dy2[24] = 2 * pZ[3];
		pXpYpZ = pX[1] * pZ[3];
		table[49] = pXpYpZ;
		table_df_dx[37] = 2 * pXpYpZ;
